# Libvirt 3 Hosts, 1 Hub + 2 OCP Fleet

**Install type:** connected through an explicit forward proxy
(`Environment.spec.ocpInstall.restricted.proxy`).

Three libvirt provider hosts each run a single multi-node cluster: a 3-node
...
| --- | --- | --- | --- | --- |
| `libvirt-3-hosts-hub` | `libvirt-3-hosts-hub-provider` | `hub-host` | hub | multi-node (3 nodes) |
| `libvirt-3-hosts-ocp-01` | `libvirt-3-hosts-ocp-01-provider` | `ocp-01-host` | managed | multi-node (3 nodes) |
| `libvirt-3-hosts-ocp-02` | `libvirt-3-hosts-ocp-02-provider` | `ocp-02-host` | managed | multi-node (3 nodes) |
...
| `libvirt-3-hosts-hub-fleet-bmc-credentials` | Redfish emulator credentials (shared across the three providers) |
...
gitups secrets generate -f test/e2e/libvirt-3-hosts-1-hub-2-ocp-fleet
```
...
```text
make e2e-dry-run CASE=libvirt-3-hosts-1-hub-2-ocp-fleet
make e2e CASE=libvirt-3-hosts-1-hub-2-ocp-fleet
```
