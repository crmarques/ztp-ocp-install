...
mkdir -p "$OUT/test4"
cp tests/e2e/test4/gitops-package-set.yaml "$OUT/test4/gitops-package-set.yaml"
```
...
./bin/gitups check    test4 -d "$OUT"
./bin/gitups render test4 -d "$OUT" --context test4 --prune
```
...
## 5. GitOpsPackageSet the tokens declarest's CRs reference
...
are **not** generated by this GitOpsPackageSet — authoring them is the
